...
import { CheckCircle2, Anchor, Shield } from "lucide-react";

export default function UpdateFishermanForm({ darkMode }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      setSubmitting(true);
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/api/fisherman/${id}`,
        payload,
        { headers: { Authorization: "Bearer " + token } }
      );
      toast.success("Fisherman updated successfully");
      navigate("/admin/fishermen");
    } catch (err) {
      console.error(err);
      toast.error(err?.response?.data?.message || "Fisherman updating failed");
    } finally {
      setSubmitting(false);
    }
  }
      ...
      >
        ...
        <div className="px-6 pt-6">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <span
              className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${
                isDisabled ? "bg-slate-400 text-white" : "bg-emerald-600 text-white"
              }`}
            >
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            ...
            >
              {submitting ? "Updating..." : "Update Fisherman"}
            </button>
          </div>
        </form>
      </div>
