...
//                         }`}
//                         title="Export all equipment as CSV"
//                     >
...
import { Link, useNavigate } from "react-router-dom";
import axios from "../../api/axios";
import { Package, Trash2, Edit, PlusCircle, Download, Search, Filter, X } from "lucide-react";
import toast from "react-hot-toast";
...
export default function EquipmentManagement({ darkMode }) {
        ...
        if (availabilityFilter !== "all") {
            switch (availabilityFilter) {
                case "available":
                    ...
                    break;
                case "low-stock":
                    filtered = filtered.filter(item => item.availableQuantity > 0 && item.availableQuantity < item.totalQuantity);
                    break;
                case "out-of-stock":
                    filtered = filtered.filter(item => item.availableQuantity === 0);
                    break;
    ...
    const getMaintenanceCounts = () => {
        const counts = { all: equipment.length };
        counts.requires = equipment.filter(item => item.requiresMaintenance).length;
        counts.noMaintenance = equipment.length - counts.requires;
        return counts;
    };
            ...
            <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        type="button"
                        className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition-all ${darkMode ? "bg-slate-700 text-white hover:bg-slate-600" : "bg-slate-100 text-slate-800 hover:bg-slate-200"}`}
            ...
            <div className={`mb-6 p-4 rounded-lg ${darkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <div className="flex-1 relative">
                        <Search className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${darkMode ? 'text-slate-400' : 'text-gray-400'}`} size={20} />
                        <input
                            type="text"
                            placeholder="Search equipment by name, ID, or description..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className={`w-full pl-10 pr-4 py-2 rounded-lg border ${darkMode ? 'bg-slate-700 border-slate-600 text-slate-100 placeholder-slate-400' : 'bg-white border-gray-300 placeholder-gray-400'}`}
                        />
                    </div>
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg border ${darkMode ? 'bg-slate-700 border-slate-600 hover:bg-slate-600' : 'bg-white border-gray-300 hover:bg-gray-50'}`}
                    >
                        <Filter size={18} />
                        Filters
                        {(typeFilter !== 'all' || availabilityFilter !== 'all' || maintenanceFilter !== 'all') && (
                    ...
                    {(searchTerm || typeFilter !== 'all' || availabilityFilter !== 'all' || maintenanceFilter !== 'all') && (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg ${darkMode ? 'bg-red-700 hover:bg-red-600 text-white' : 'bg-red-500 hover:bg-red-600 text-white'}`}
                        >
                            <X size={18} />
                            Clear
                        </button>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {[
                                    ...
                                    <button
                                        ...
                                        onClick={() => setAvailabilityFilter(availability.value)}
                                        className={`px-3 py-1 rounded-full text-sm font-medium transition-all ${
                                            availabilityFilter === availability.value
                                                ? (darkMode ? 'bg-cyan-600 text-white' : 'bg-blue-600 text-white')
                                                : (darkMode ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300')
                                        }`}
                                    >
                                        {availability.label} ({availabilityCounts[availability.value] || 0})
                                    </button>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {[
                                    ...
                                    <button
                                        ...
                                        onClick={() => setMaintenanceFilter(maintenance.value)}
                                        className={`px-3 py-1 rounded-full text-sm font-medium transition-all ${
                                            maintenanceFilter === maintenance.value
                                                ? (darkMode ? 'bg-cyan-600 text-white' : 'bg-blue-600 text-white')
                                                : (darkMode ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300')
                                        }`}
                                    >
                                        {maintenance.label} ({maintenanceCounts[maintenance.value] || 0})
                                    </button>
                                ))}
                            </div>
                        </div>
            ...
            {loading ? (
                ...
                <div className={`rounded-lg p-8 text-center ${darkMode ? 'bg-slate-800' : 'bg-gray-100'}`}> 
                    ...
                    {(searchTerm || typeFilter !== 'all' || availabilityFilter !== 'all' || maintenanceFilter !== 'all') && (
                        <button 
                            onClick={clearFilters}
                            className={`mr-2 px-4 py-2 rounded-lg ${darkMode ? 'bg-slate-700 hover:bg-slate-600 text-white' : 'bg-gray-300 hover:bg-gray-400 text-gray-700'}`}
                        >
                            Clear Filters
                        </button>
                    )}
