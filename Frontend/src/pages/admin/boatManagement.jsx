...
import { Link, useNavigate } from "react-router-dom";
import api from "../../api/axios";
import { Ship, Trash2, Edit, PlusCircle, Download, Search, Filter, X } from "lucide-react";
import toast from "react-hot-toast";
...
export default function BoatsManagement({ darkMode }) {
    ...
    const handleExportCSV = () => {
        ...
        const headers = ["Boat Number", "Name", "Capacity", "Status", "Equipment Count"];
        const escapeCSV = (str) => {
            if (str == null) return "";
            str = String(str);
            if (str.includes(',') || str.includes('"') || str.includes('\n')) {
        ...
        const rows = filteredBoats.map((boat) => [
            escapeCSV(boat.boatNumber),
            escapeCSV(boat.name),
            escapeCSV(boat.capacity),
            escapeCSV(boat.status),
            boat.equipmentAssignments?.length || 0
        ]);
        const csv = [headers.join(","), ...rows.map((r) => r.join(","))].join("\n");
        const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
        const url = URL.createObjectURL(blob);
        ...
        <div className={`p-6 ${darkMode ? 'bg-slate-900 text-slate-100' : 'bg-white text-slate-800'}`}>
            <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        title="Export filtered boats as CSV"
                    >
                        <Download className="h-4 w-4" />
                        Export
                    </button>
            ...
            {loading ? (
                ...
                <div className={`${darkMode ? 'bg-slate-800 text-slate-200' : 'bg-gray-100 text-gray-600'} rounded-lg p-8 text-center`}>
                    ...
                    <p className={`mb-4 ${darkMode ? 'text-slate-300' : 'text-gray-500'}`}>
                        {boats.length === 0 
