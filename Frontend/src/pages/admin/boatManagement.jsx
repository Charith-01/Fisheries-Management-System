...
export default function BoatsManagement({ darkMode }) {
  ...
  // ✅ Export boats to PDF (replaces CSV)
  ...
  return (
    <div className={`p-6 ${darkMode ? "bg-slate-900 text-slate-100" : "bg-white text-slate-800"}`}>
      ...
      {loading ? (
        ...
        <div
          className={`${
            darkMode ? "bg-slate-800 text-slate-200" : "bg-gray-100 text-gray-600"
                  ...
                  <td className="py-3 px-4">
                    <Link to={`/admin/boats/${boat.boatNumber}`}>
                      {boat.images && boat.images.length > 0 ? (
                        <img
                          src={
                            boat.images[0].startsWith("http")
                              ? boat.images[0]
                              : `http://localhost:3000${boat.images[0]}`
                          }
                          alt={`${boat.name} boat`}
