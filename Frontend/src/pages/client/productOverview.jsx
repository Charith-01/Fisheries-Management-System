...
export default function ProductOverview() {
  ...
  useEffect(() => {
    if (status === "loading") {
      axios
        .get(import.meta.env.VITE_BACKEND_URL + "/api/product/get/" + encodeURIComponent(params.id))
        .then(async (res) => {
          const base = res.data?.product ?? res.data?.data ?? res.data;
          if (!base) throw new Error("Empty product payload");

          // Enrich with stock data from /all (which now returns stockWeight/stockUnit/stockType based on FishStock ObjectId linking)
          try {
          ...
          <div className="w-[50%] h-full">
            <div className="h-full w-full p-6 md:p-8">
              ...
              {(() => {
                ...
                const effectiveStock = typeof product?.stockWeight === "number" ? product.stockWeight : product?.stock;
                      ...
                      {hasDiscount && (
                        <>
                          <div className="text-sm line-through text-slate-400">
                            Rs.{" "}
                            {product.labeledPrice.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </div>
                      ...
                      >
                        ...
                        {typeof effectiveStock === "number" && (
                          <span className="ml-1 text-slate-500">
                            ({effectiveStock} {effectiveUnit || product?.unit} left)
                          </span>
                        )}
                      </span>
                    </div>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <span className="self-center text-sm text-slate-600 ml-1">{effectiveUnit ?? "unit"}</span>
                    </>
                  );
                })()}
              </div>
              ...
              <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
                <button
                  ...
                  onClick={() => {
                    ...
                    toast.success(
                      `Added ${qty} ${effectiveUnit || ""} (Rs. ${total.toLocaleString(undefined, {
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const total = qty * price;
                    toast(
                      `Checkout: ${qty} ${effectiveUnit || ""} • Total Rs. ${total.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                      })}`
                    );
                  }}
              ...
              <div className="flex-1">
                ...
                <details className="mt-3 text-xs text-red-800/80">
                  ...
                  <ul className="list-disc ml-5 mt-2 space-y-1">
                    ...
                    <li>
                      Ensure the server is running and <code>VITE_BACKEND_URL</code> is set.
                    </li>
                  </ul>
                </details>
              </div>
